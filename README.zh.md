...
第一版瘦身会删除各包的 `src/`，理由是"published 包运行 `lib/`，`src` 只是源码"。**这个理由是错的。**

`koffi` 包的 `index.js` 正文只有一行：

```js
export { default } from "./src/koffi/index.js";
```

**入口在包根、真正代码在 `src/`。** 删掉它会让插件树加载失败，用户装完直接打不开。这个坑让 CI 白跑了两轮。

现在 `src/` 瘦身**默认关闭**，需要时用 `--prune-src`（或 workflow 的 `prune_src` 输入）显式开启。
开启后仍需通过三条保守判断才会删（入口字段不指向 `src/`、无相对导入 `src/`、`src/` 内无可加载文件）。
保留它的判断逻辑是为了将来真有需要时不至于从零再来，**但默认值就是"一个字节都不动"**。

另外 staging 阶段新增了一条**确定性断言**：读 `koffi/index.js` 的相对导出，逐个确认目标文件仍在树里；
缺失就直接在 staging 步骤报错，并提示"是瘦身删了运行时代码"——不再让它以"启动失败"的形态出现。
